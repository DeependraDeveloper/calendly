...
import {
  ...
  upsertAvailableSlot,
} from "../repositories/slot.reposiotry.js";
...
export async function regenerateHostSlots(input: RegenerateHostSlotsInput) {
  ...
  const [rules, exceptions, eventTypes, bookedSlots] = await Promise.all([
   ...
   console.log("availablity", rules)
      console.log("holiday", exceptions)
                console.log("services", eventTypes)
                         console.log("bookings", bookedSlots)
  ...
  for (const eventType of eventTypes) {
    ...
    for (let cursor = from; cursor <= to; cursor = cursor.plus({ days: 1 })) {
      ...
      console.log(dayExceptions,"day exceptions")
      ...
      const dayExceptionsWithTimeZone = dayExceptions.map((ex) => ({
        ...
        timeZone: ex.timezone,
            ...
            console.log(dayExceptionsWithTimeZone,"day exceptions with timezone")
