...
export async function excahngeSetupCode(code: string) {
  const client = getGoogleOauthClient();
...
export async function createCalendarEvent(bookingId: number) {
  const booking = await findBookingById(bookingId);

  if (!booking || booking.status !== "CONFIRMED")
    throw notFound("Booking nof found or not confirmed");
